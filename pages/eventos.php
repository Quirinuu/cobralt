...
if (pb_render_managed_page_if_exists('eventos', 'eventos')) { exit; }
...
$pastColts = colt_editions_newest_first();
...
<section class="section" id="realizados" style="background:linear-gradient(180deg,var(--off-white) 0%,#fff 100%);" aria-labelledby="realizados-title">
  <div class="section-inner" style="max-width:1180px;">
    ...
    <div class="events-carousel-wrapper" data-animate>
      <div class="events-carousel is-overflow" id="eventsCarousel">
...
.past-event-stats span { font-size:0.73rem; color:var(--slate-600); display:flex; align-items:flex-start; gap:6px; line-height:1.35; }
.past-event-stats span::before { content:''; width:4px; height:4px; background:var(--sky); border-radius:50%; flex:0 0 auto; margin-top:0.42em; }
...
(function(){
  ...
  for (var i = 0; i < total; i++) {
    ...
    d.setAttribute('aria-label', 'CoLT ' + (i + 1));
    (function(idx){ d.addEventListener('click', function(){ goTo(idx); }); })(i);
    dotsWrap.appendChild(d);
    dots.push(d);
  }
  function cardStep() {
    return cards[0] ? cards[0].offsetWidth + 20 : 1;
  }
  function updateDots() {
  ...
  function goTo(i) {
    ...
    carousel.scrollTo({ left: current * cardStep(), behavior: 'smooth' });
    updateDots();
  }
  if (prevBtn) prevBtn.addEventListener('click', function(){ goTo(current - 1); });
  if (nextBtn) nextBtn.addEventListener('click', function(){ goTo(current + 1); });
  carousel.addEventListener('scroll', function(){
    var idx = Math.round(carousel.scrollLeft / cardStep());
    if (idx !== current) {
